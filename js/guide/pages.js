...
const CARDS = {
  ...
  'doghouse': {img:'doghouse', to:'m-doghouse', t:'Doghouse Shuffles', s:'Get out of the Doghouse —<br>if you can.', alt:'Doghouse Shuffles — Get out of the Doghouse — if you can.'},
  ...
  'user-guide': {img:'user-guide', to:'home', t:'TournaQ User Guide', s:'Every path through the app, from the<br>first tap to the finished scorecard.', alt:'TournaQ User Guide — Every path through the app, from the first tap to the finished scorecard.', k:'Every path through the app.'},
};
...
home: {
  ...
  blocks:[
    ...
    panel('The way through','What each stop on the map does.',[
      item('i-admin','Administration','Players, teams and groups, set up once — or added while you build the tournament.'),
...
administration: {
  ...
  h1:['Administration'],
  lead:'The admin area is there to get your games and tournaments ready with as little friction as possible. Set up players, set up teams, put players into teams — or create both in one go — and sort them into groups you can filter by later, so picking a field takes seconds. Do it by hand on your phone before anything starts, or export a template, fill it in as a spreadsheet and import the whole club at once.',
  blocks:[
...
'admin-hand': {
  ...
  blocks:[
    ...
    shot('guide/01_admin/14_players_gender_filter',430,1727,[430, 645],
      'Filtering the pool',
...
arena: {
  ...
  blocks:[
    ...
    fbox('i-score','Every mode comes with the scorecard it needs','Team competitions get the classic scorecard with sets and target points. The scramble competitions get one of two, depending on how the round is scored — the timed card, or the one that shows the queue.', [
      ...
      {icon:'i-queue', title:'Queue scorecard', body:'Royal Shuffles, Doghouse Shuffles, Royal Duos'},
    ]),
  ],
...
'quick-game': {
  ...
  eyebrow:'Standalone',
  h1:['Quick ','Game'],
  lead:'Quick access to a scorecard: you are on one within seconds and counting straight away. For every situation where somebody lost count again, where the numbers scratched into the sand stopped making sense the moment the sides swapped, or where nobody agrees on who scored last and who serves now. Or simply because it is relaxed, and it is fun.',
  blocks:[
  ...
  inbox:[
    ...
    shot('guide/02_quick_game/01_games',430,932,[430, 860],
      'Your matches',
...
'queue-modes': {
  ...
  blocks:[
    ...
    panel('Scoring & ejection','What wins a game on court, and what sends a side back into the queue. The first three rules hold in every queue mode — only the target and the price of a conceded point differ between Royals and Doghouse Shuffles.',[
  ...
  next:['m-royal-shuffle','m-royal-rotation','m-doghouse','m-royal-duo','sc-queue'],
},
...
'm-royal-duo': {
  ...
  eyebrow:'Queue mode · team competition',
  h1:['Royal ','Duos'],
  lead:'King of the Court played in fixed pairs. You enter as a duo and keep that partner for the whole tournament, so the ranking is by team rather than by player. This is a queue mode: if you have not read how the queue, the ejections and the ranking work, start with ' + pageLink('queue-modes','Queue Modes') + '.',
  blocks:[
    panel('How it works',null,[
      step(1,'i-people','You enter as a pair','Your partner is settled before the first whistle and does not change again.'),
      step(2,'i-crown','Hold the court','Win and you stay. Lose and the duo goes back to the queue.'),
      step(3,'i-timer','The round ends on the clock','The duos are spread across the courts again — new opponents, same partner.'),
      step(4,'i-trophy','Team standings','Every point and every game is credited to the duo, not to the two players separately.', {tone:'tint'}),
    ]),
    fbox('i-sync','The same engine as Royal Rotations, with one setting changed','Royal Duos runs on Royal Rotations’ pages — the same setup form, the same tournament page, the same queue scorecard. What differs is that the pairs are fixed instead of redrawn every round, so anywhere Royal Rotations says “player”, this mode says “duo”.'),
    sect('Where the duos come from', 'i-people'),
    panel('Pairing','Set at setup, and fixed once the tournament begins. Which unit you count in is your choice — shuffling is a question about people, picking is a question about pairs.',[
      item('i-swap','Shuffle','Hand it a list of players and it draws the duos for you. Redraw the whole set, or swap a single pairing, right up until the tournament starts.'),
      item('i-admin','Pick teams','Enter the duos yourself — from your team hub in Administration, or built here on the spot.'),
    ], {id:'pairing'}),
    fbox('i-shield','If a partner has to leave','There are no stand-ins in Royal Duos: half a duo is not a competitor. Sit the pair out for a single round and it is back for the next one, or take it out of the tournament for good — either way it keeps the points it has already earned.'),
    sect('The three steps', 'i-south'),
    grid([
      {icon:'i-trophy', label:'Set it up', cap:'Duos, courts, format — and what it costs in time.', to:'m-royal-duo-hub'},
      {icon:'i-grid', label:'Run it', cap:'The live screen: standings, schedule, courts.', to:'m-royal-duo-run'},
      {icon:'i-score', label:'Score it', cap:'The card you score on, and what it shows.', to:'m-royal-duo-score'},
    ]),
  ],
...
'm-royal-duo-hub': {
  title:'Setting up a Royal Duo', route:'/guide/modes/royal-duo/tournament-hub', icon:'i-trophy', parent:'m-royal-duo',
  eyebrow:'Royal Duo · Tournament Hub',
  h1:['Setting up a ','Royal Duo'],
  lead:'Build the duos, set the courts, the rounds and the round length, and go. TournaQ works out how many rounds fit in the time you have.',
  blocks:[
    panel('What you set',null,[
      item('i-people','The duos','One roster section, not two. Shuffle a list of players into pairs, or pick the teams yourself.'),
      item('i-crown','The king half','The strike target, the assignment mode and the auto-eject threshold.'),
      item('i-clock','One form for both','Every field feeds the Schedule Preview, so the finish time moves as you decide.'),
    ]),
    fbox('i-sync','Every duo has to be whole before you can start','Create stays disabled while anyone on the roster is still unpaired — a loose player has no competitor to be part of. A duo that is one player *short* is fine: its empty seat plays as a placeholder, and the team scores like any other.'),
    shot('guide/05_royal_duo/03_setup_full',430,1435,[430, 645],
      'The whole setup page',
      'Every choice on one screen, scrolled out in full. Defaults are already sensible — in practice you set the people, the courts and the time, and leave the rest alone.',
      'The complete Royal Duo setup page'),
    shot('guide/05_royal_duo/20_teams_sheet',430,1077,[430, 645],
      'The duos',
      'What makes this its own mode: partners are fixed for the whole session. Pair the roster up here, or bring standing teams over from Administration.',
      'The Royal Duo teams sheet listing standing pairs'),
    sect('Every setting on the page','i-south'),
    opts('Royal Duo · Tournament Setup — the page is titled “Royal Duo” in the app',[
      opt('Target Players','8, 12, 16, 20, 24, 32, 48 — or type any number','16 · from twice the side size, up to 128',
        {what:'How many people you are planning for. The duos are built out of them, so at 2vs2 this is twice the number of teams.',
      ...
      opt('Style','2vs2 through 6vs6','2vs2 · 2 to 6 a side',
        {what:'How many players a side fields — and so how many players make up one duo.',
         when:'A size stays visible but unselectable once the planned field cannot seat two full sides of it.',
      ...
      opt('Rounds','3, 4, 5, 6, 8, 10, 12 — or type any number','6 · 1 to 999',
        {what:'How often the standing duos are spread across the courts again. The partners do not change — the opponents do.',
      ...
      opt('Strike Points','0, 3, 5, 7, 10, 15, 21 — or type any number','5 · 0 to 999',
        {what:'What it takes to win the court and leave it as the winning duo.',
      ...
      opt('Assignment','Manual · Automated · Auto-Allplay','Manual',
        {what:'How the next duo gets onto the court.',
      ...
      opt('Auto-eject challenger','0, 1, 2, 3','0 — off · 0 to 3',
        {what:'Ends a hopeless challenge early instead of watching it out.',
         help:'“Prompts you to eject the challenger team once the on-court team scores this many points against them. 0 keeps it off — you can still eject by hand anytime.”'}),
      opt('Odd player handling','—','Not on this page',
        {what:'Royal Rotations carries a Placeholder / Jumper choice for the player a reshuffle cannot pair. Royal Duos has no such player: the pairs are settled before play, so the question never arises.'}),
    ]),
    sect('The duos','i-people'),
    opts('The Duos section — one roster, in whichever unit you chose',[
      opt('Pairing','Shuffle · Pick teams','Shuffle',
        {what:'Where the standing pairs come from.',
         help:'“Shuffle takes a list of players and draws the duos for you — you can redraw the whole set or swap a single pairing until the tournament starts. Pick teams lets you enter the duos yourself, from your team hub or built here on the spot. Either way the pairs are fixed once the tournament begins: partners never change, and the ranking is by team.”'}),
      opt('Pair up players','The roster, drawn into duos','Shuffle only',
        {what:'The list of entrants and the pairs drawn from it.',
         when:'“{count} players still unpaired” while anyone is loose — and Create stays disabled until the count reaches zero.'}),
      opt('Draw again','Redraws every pairing','Shuffle only',
        {what:'A fresh draw across the whole roster. Available until the tournament starts.'}),
      opt('Swap {name} with','Exchanges one player between two duos','Shuffle only',
        {what:'Fixes the one pairing nobody is happy with, without redrawing the rest.'}),
      opt('Add existing teams','Standing teams from Administration','Pick teams only',
        {what:'Brings pairs you have already saved straight in, names and all.'}),
      opt('Create a duo','Builds one pair here','Pick teams only',
        {what:'“Pick {count} players for this duo.” — with an optional duo name.'}),
      opt('{count} duos','The running count','Always',
        {when:'“{count} duos are a player short and play with a placeholder.” — a warning, not a block. A short duo plays and scores like any other.'}),
    ], ['Control','What it does','When you see it']),
    fbox('i-check','What the page refuses, and what it only warns about','Some of these block Create outright; the rest are advice under the schedule.',[
      {icon:'i-people', title:'Somebody is still unpaired', body:'Create stays disabled until every entrant sits in a duo. A short duo is allowed; a loose player is not.'},
      ...
      {icon:'i-swap', title:'How much mixing you get, counted in duos', body:'“At {rounds} rounds each duo shares a court with about {met} of the other {total}. {target} rounds makes it about {more}.” — with a one-tap “Use {target} rounds”.'},
      {icon:'i-star', title:'Full coverage reached', body:'“At {rounds} rounds every duo shares a court with all {total} others.”'},
      {icon:'i-sync', title:'No fresh-partner promise here', body:'Royal Rotations nudges you about repeat partners. Royal Duos cannot — the partner is the point.'},
    ]),
    sect('The rest of the page','i-south'),
    opts('Below the grid — Schedule Preview, name',[
      ...
      opt('Tournament Name','Free text, plus a suggest button','A random name in the mode’s own style',
        {when:'Create stays disabled while the name is empty.',
         help:'“Suggest a name”'}),
      opt('Create Tournament','The button that ends setup','Disabled until the page is ready',
        {what:'Needs a name, a match duration above zero, at least one round, every entrant in a duo, and a plan that can be built — every court you asked for has to reach its player floor.',
         when:'The line above it reads “Ready to start!” or “Setup looks good!” — otherwise it names what is still missing.'}),
    ]),
  ],
  next:['m-royal-duo-run','tournament-hub'],
},

'm-royal-duo-run': {
  title:'Running a Royal Duo', route:'/guide/modes/royal-duo/tournament', icon:'i-grid', parent:'m-royal-duo',
  eyebrow:'Royal Duo · Running it',
  h1:['Running a ','Royal Duo'],
  lead:'One screen carries several courts at once: how far along you are, which duos are where, who is queuing, and when you will finish.',
  blocks:[
    panel('What the screen gives you',null,[
      item('i-target','Progress and settings in one row','Rounds complete, a progress ring and a row of pills — standings, allocation, duos, strike target, assignment and estimated finish.'),
      ...
      item('i-trophy','The table ranks duos','Every metric on the standings page is a team total. A duo that retires keeps its figures and is flagged, the same way an ejected player stays in an individual table.'),
    ]),
    shot('guide/05_royal_duo/04_overview',430,932,[430, 860],
      'The tournament page',
      'Rounds, courts and the queue — the same shape as Royal Rotation, except the names beside each other never change.',
      'A running Royal Duo'),
    shot('guide/05_royal_duo/06_overview_by_court',430,1818,[430, 645],
      'Read it by court',
      'The same session grouped by court.',
      'A Royal Duo schedule grouped by court'),
    shot('guide/05_royal_duo/15_court_allocation',430,932,[430, 645],
      'Court allocation',
    ...
    opts('Royal Duo · the pill row',[
      opt('“Standings”','The overall ranking, by duo — and the page where the ranking method itself can still be changed.','Opens the ranking page'),
      ...
      opt('“Duos”','The standing pairs, and each one’s record so far.','Opens the sheet'),
      ...
      opt('“{n} rounds”','How many times the duos are spread across fresh courts.','Edit here — opens the format sheet'),
      opt('“{n} courts”','How many independent queues run at once.','Edit here — opens the format sheet'),
      opt('“{n} pt strike”','What it takes to win the court and leave it as the winning duo. Hidden when strike points are switched off.','Edit here — opens the format sheet'),
      ...
      opt('“Manual” / “Automated” / “Auto-Allplay”','How the next duo gets onto the court.','Edit here — opens the format sheet'),
    ...
    fbox('i-shield','Taking a duo out','The eject sheet asks which kind: “Sit out this round” skips the round in flight and brings the pair back for the next one; “Remove from tournament” stops it playing for good. Either way it keeps the points it has earned. There is no stand-in option — half a duo is not a competitor.'),
    sect('The rest of the page','i-south'),
    grid([
...
'm-royal-duo-score': {
  title:'Scoring a Royal Duo match', route:'/guide/modes/royal-duo/scorecard', icon:'i-score', parent:'m-royal-duo',
  eyebrow:'Royal Duo · Scorecard',
  h1:['Scoring a ','Royal Duo match'],
  lead:'Each court runs its own king: the duo on, the duo challenging, and the queue behind them, with the scoring controls under your thumb.',
  blocks:[
    ...
    fbox('i-queue','Which card this is','The queue scorecard — the court, the challengers and the queue behind them, with the round clock running in the header. The names beside each other never change, so a duo is read as one unit all the way down the card.'),
    shot('guide/05_royal_duo/07_court_manual_assigning',430,1826,[430, 645],
      'Manual: choosing who comes on',
      'With manual assignment nobody is seated for you. The card offers the fairest pairings it can find on the left and the whole queue on the right — one tap on a suggestion starts that side immediately.',
      'A Royal Duo court with the manual side picker open'),
    shot('guide/05_royal_duo/08_court_manual_assigning_landscape',860,397,[860, 1600],
      'The picker, sideways',
      'Landscape keeps the suggestions and the roster side by side.',
      'The Royal Duo manual picker in landscape'),
    shot('guide/05_royal_duo/09_court_manual_running',430,1826,[430, 645],
      'Manual: the court running',
      'Once a side is seated the picker gives way to the score. There is no challenger queue here — in manual assignment you decide who comes on next, so the card shows only the game in front of you.',
      'A Royal Duo court running under manual assignment'),
    shot('guide/05_royal_duo/10_court_manual_running_landscape',860,397,[860, 1600],
      'Running, sideways',
      'The same court in landscape, with the eject rail on the edge.',
      'A running Royal Duo court in landscape'),
    shot('guide/05_royal_duo/11_court_automated',430,2030,[430, 645],
      'Automated: the queue picks',
      'Automated assignment fills the chain for you: who is on court, who challenges next, who is up after them. Re-roll if a pairing looks wrong, and the rest of the queue shuffles up behind it.',
      'A Royal Duo court with the automated challenger queue'),
    shot('guide/05_royal_duo/12_court_automated_landscape',860,397,[860, 1600],
      'The queue, sideways',
      'Landscape stacks the chain beside the score instead of above it.',
      'An automated Royal Duo court in landscape'),
    shot('guide/05_royal_duo/13_court_allplay',430,1780,[430, 645],
      'Auto All-Play',
      ...
      'A Royal Duo court in Auto All-Play with its scorekeeper'),
    shot('guide/05_royal_duo/14_court_allplay_landscape',860,397,[860, 1600],
      'All-Play, sideways',
      'The full chain and the scorekeeper, laid out for the sideline.',
      'An Auto All-Play Royal Duo court in landscape'),
    shot('guide/05_royal_duo/23_court_time_up',430,932,[430, 645],
      'When the clock runs out',
      'A queue court has no "time is up" prompt to answer. The clock hitting zero closes the court outright and puts its ranking on screen — who won the court, and how everyone below them finished. Portrait only: landscape shows the same card, wider.',
      'A Royal Duo court closed by its timer, showing the court ranking'),
    sect('What this card gives you','i-south'),
    opts('Royal Duo · the scorecard',[
      opt('The pill row','Standings and Allocation, duos, the side size, rounds, courts, strike points, auto-eject and assignment — plus this round and this court. The format pills edit here only on a single-court session','Under the title'),
      opt('Court','The duo holding the court right now, and the points they have made this turn.','Always'),
      opt('Challengers','The duo waiting to come on next.','Always — “Waiting for players…” while the queue is too thin'),
      opt('Up Next','The stage behind the challengers, with “Re-roll” to draw a different order.','Automatic assignment — “Not enough players in queue.” when there is nobody left to draw'),
      opt('Score buttons','Count the points the duo on court is making.','Always'),
      opt('Session timer','The round’s countdown, with Stop, Resume and Start / Restart.','Always'),
      opt('Eject','Sends the duo off and brings the next one on. “Eject / Challenger” where the prompt can rotate a challenger instead.','Always — manual ejection stays available whatever the automatic thresholds say'),
      ...
      opt('Take this duo out','“Sit out this round” — the duo skips the round in flight and is back for the next one. “Remove from tournament” — it stops playing and keeps the points it has earned so far.','From a duo on the card. No stand-ins: half a duo is not a competitor'),
      opt('Undo','Takes the last step back. With points already on the board it asks first: “The team currently on court has {points} point(s) recorded. Undoing will discard them.”','After an ejection, and during a manual pick'),
      opt('Change referee','“Select who referees this court. The current one returns to the queue.” The successor is suggested from the current court team.','Auto-Allplay, where a rotating referee keeps score'),
      opt('Start / Finish court','“Start Court” drops it into live play; “Finish court” asks “Record this court’s results now and end its round.” and is reversible with “Undo Finish”.','Bottom of the card'),
      opt('App-bar QR menu','“Export court” hands the whole court to another phone; “Manually Set Score” writes the results in directly.','While the court is live'),
...
'm-league-hub': {
  ...
  blocks:[
    ...
    shot('guide/08_league/03_setup_full',430,1092,[430, 645],
      'The whole setup page',
      'Every choice on one screen, scrolled out in full. Defaults are already sensible — in practice you set the people, the courts and the time, and leave the rest alone.',
      'The complete League setup page'),
    sect('Every setting on the page','i-south'),
...
'm-league-run': {
  ...
  blocks:[
    ...
    shot('guide/08_league/04_table',430,932,[430, 860],
      'The tournament page',
      'The row of pills under the header is the whole event at a glance — and most of them are also the way in: Standings, Crosstable and Allocation open, the padlocked ones tell you what is already fixed.',
      'A running League with its pill row and schedule'),
    shot('guide/08_league/06_schedule_by_round',430,2888,[430, 645],
      'Three ways to read the schedule',
      'Time, Round or Court. The same fixtures, grouped the way you happen to need them — by when they start, by the round they belong to, or by which court to walk to.',
      'The League schedule grouped by round'),
    shot('guide/08_league/12_crosstable',860,505,[860, 1600],
      'The crosstable',
      'League’s own view, and the one no other mode has: every team against every other, with the head-to-head result in the cell. Blank cells are fixtures still to come.',
...
'm-league-score': {
  ...
  blocks:[
    ...
    shot('guide/08_league/08_scorecard',430,998,[430, 645],
      ...
      'A best-of-three match part-way through: the first set decided, the second one running under the thumbs, the third still to come. Tap a set in the strip to look back at it without losing the live score.',
      'A League scorecard mid second set of three'),
    shot('guide/08_league/09_scorecard_landscape',860,397,[860, 1600],
      'Turned sideways',
      'Rotate the phone and the same match fills the width — bigger numbers, both thumbs still in reach. Nothing is hidden in landscape; it is the same card, laid out for a court-side glance.',
      'The same League scorecard in landscape'),
    sect('What this card gives you','i-south'),
...
'm-elimination-hub': {
  ...
  blocks:[
    ...
    shot('guide/09_elimination_single/03_setup_full',430,1143,[430, 645],
      'The whole setup page',
      'Every choice on one screen, scrolled out in full. Defaults are already sensible — in practice you set the people, the courts and the time, and leave the rest alone.',
      'The complete Elimination setup page'),
    sect('Every setting on the page','i-south'),
...
'm-elimination-run': {
  ...
  blocks:[
    ...
    shot('guide/09_elimination_single/04_bracket',430,932,[430, 860],
      'The tournament page',
      'The draw as a list, newest round first, with the pill row above it carrying the event’s shape and its way into standings, the bracket and court allocation.',
      'A running single-elimination event'),
    shot('guide/09_elimination_single/11_standings_bracket',860,477,[860],
      'The bracket',
      'The same draw drawn as a tree. Winners feed to the right, and a seat reads TBD until the match below it is decided. Pinch, or use the buttons in the corner, to move around a big draw.',
      'A single-elimination bracket with the quarter-finals decided'),
    shot('guide/10_elimination_double/04_bracket_canvas',860,516,[860, 1600],
      'Double elimination',
      'The same view with a losers bracket underneath: one defeat drops a team to the lower band rather than out, and the two bands meet again at the grand final.',
...
'm-elimination-score': {
  ...
  blocks:[
    ...
    fbox('i-check','Which card this is','The classic scorecard — sets, target points and side switches. The same one every team competition uses.'),
    shot('guide/09_elimination_single/07_scorecard',430,1027,[430, 645],
      'The scorecard, top to bottom',
      'A best-of-three match part-way through: the first set decided, the second one running under the thumbs, the third still to come. Tap a set in the strip to look back at it without losing the live score.',
      'A Elimination scorecard mid second set of three'),
    shot('guide/09_elimination_single/08_scorecard_landscape',860,397,[860, 1600],
      'Turned sideways',
      'Rotate the phone and the same match fills the width — bigger numbers, both thumbs still in reach. Nothing is hidden in landscape; it is the same card, laid out for a court-side glance.',
      'The same Elimination scorecard in landscape'),
    sect('What this card gives you','i-south'),
...
'm-classic-hub': {
  ...
  blocks:[
    ...
    shot('guide/11_tournaq_classic/03_setup_full',430,1815,[430, 645],
      'The whole setup page',
      'Every choice on one screen, scrolled out in full. Defaults are already sensible — in practice you set the people, the courts and the time, and leave the rest alone.',
      'The complete TournaQ Classic setup page'),
    sect('Every setting on the page','i-south'),
...
'm-classic-run': {
  ...
  blocks:[
    ...
    shot('guide/11_tournaq_classic/04_schedule',430,932,[430, 860],
      'The tournament page',
      'A Classic runs in two halves, and the page shows whichever one you are in — group fixtures first, then the knockout draw once the groups have decided who goes where.',
      'A running TournaQ Classic during its group stage'),
    shot('guide/11_tournaq_classic/10_standings_table',430,992,[430, 645],
      'Group standings',
      'One table per group, in the order that decides qualification. The positions that go through are the ones you picked at setup.',
      'TournaQ Classic group tables mid-stage'),
    shot('guide/11_tournaq_classic/17_advanced_standings_bracket',860,646,[860, 1600],
      'Groups feeding the draw',
      'The full shape in one picture: group tables on the left, a connector from each qualifying place to the seat it fills, and a bracket per tier. Here four uneven groups feed a Gold and a Silver draw — and because one group is a team short, Silver carries a real bye.',
...
'm-classic-score': {
  ...
  blocks:[
    ...
    fbox('i-check','Which card this is','The classic scorecard — sets, target points and side switches. The same one every team competition uses.'),
    shot('guide/11_tournaq_classic/07_scorecard',430,998,[430, 645],
      'The scorecard, top to bottom',
      'A best-of-three match part-way through: the first set decided, the second one running under the thumbs, the third still to come. Tap a set in the strip to look back at it without losing the live score.',
      'A TournaQ Classic scorecard mid second set of three'),
    shot('guide/11_tournaq_classic/08_scorecard_landscape',860,397,[860, 1600],
      'Turned sideways',
      'Rotate the phone and the same match fills the width — bigger numbers, both thumbs still in reach. Nothing is hidden in landscape; it is the same card, laid out for a court-side glance.',
      'The same TournaQ Classic scorecard in landscape'),
    sect('What this card gives you','i-south'),
...
'm-swiss-hub': {
  ...
  blocks:[
    ...
    shot('guide/12_swiss_system/03_setup_full',430,1147,[430, 645],
      'The whole setup page',
      'Every choice on one screen, scrolled out in full. Defaults are already sensible — in practice you set the people, the courts and the time, and leave the rest alone.',
      'The complete Swiss System setup page'),
    sect('Every setting on the page','i-south'),
...
'm-swiss-run': {
  ...
  blocks:[
    ...
    shot('guide/12_swiss_system/04_rounds',430,932,[430, 860],
      'The tournament page',
      'Swiss draws one round at a time. Score the round you are in and the next pairing appears, built from the table as it stands — so there is never a fixture list running to the end of the day.',
      'A running Swiss System with one round scored'),
    shot('guide/12_swiss_system/10_standings_table',430,2297,[430, 645],
      'The table',
      'What the next round is drawn from. Teams on the same score meet each other, which is what keeps the pairings competitive without seeding anybody by hand.',
      'A Swiss System standings table'),
    shot('guide/12_swiss_system/11_standings_chart',860,352,[860, 1600],
      'The ladder',
      'Every team’s path through the rounds, one line each. It is the quickest way to see who has had the hard draw and who has not.',
...
'm-swiss-score': {
  ...
  blocks:[
    ...
    fbox('i-check','Which card this is','The classic scorecard — sets, target points and side switches. The same one every team competition uses.'),
    shot('guide/12_swiss_system/07_scorecard',430,1027,[430, 645],
      'The scorecard, top to bottom',
      'A best-of-three match part-way through: the first set decided, the second one running under the thumbs, the third still to come. Tap a set in the strip to look back at it without losing the live score.',
      'A Swiss System scorecard mid second set of three'),
    shot('guide/12_swiss_system/08_scorecard_landscape',860,397,[860, 1600],
      'Turned sideways',
      'Rotate the phone and the same match fills the width — bigger numbers, both thumbs still in reach. Nothing is hidden in landscape; it is the same card, laid out for a court-side glance.',
      'The same Swiss System scorecard in landscape'),
    sect('What this card gives you','i-south'),
...
'm-social-scramble-hub': {
  ...
  blocks:[
    ...
    shot('guide/03_social_scramble/03_setup_full',430,1055,[430, 645],
      'The whole setup page',
      'Every choice on one screen, scrolled out in full. Defaults are already sensible — in practice you set the people, the courts and the time, and leave the rest alone.',
      'The complete Social Scramble setup page'),
    sect('Every setting on the page','i-south'),
...
'm-social-scramble-run': {
  ...
  blocks:[
    ...
    shot('guide/03_social_scramble/04_overview',430,932,[430, 860],
      'The tournament page',
      'Rounds down the page, courts across them, and the pill row above carrying the session’s shape. Everyone’s partner changes every round, so the schedule is the mode.',
      'A running Social Scramble'),
    shot('guide/03_social_scramble/06_overview_by_court',430,1451,[430, 645],
      'Read it by court',
      'The same rounds regrouped by court, which is what you want when you are standing on one of them.',
      'A Social Scramble schedule grouped by court'),
    shot('guide/03_social_scramble/09_court_allocation',430,932,[430, 645],
      'Court allocation',
      'How many courts are in play, and when. Closing one mid-session costs players rather than minutes, and the grid prices it before you commit.',
...
'm-social-scramble-score': {
  ...
  blocks:[
    ...
    shot('guide/03_social_scramble/07_scorecard',430,1344,[430, 645],
      'The scorecard, top to bottom',
      'One game, two sides, and the round it belongs to. Scramble games are scored as a single race rather than in sets, so the card stays down to the two numbers that matter.',
      'A Social Scramble scorecard mid-game'),
    shot('guide/03_social_scramble/08_scorecard_landscape',860,397,[860, 1600],
      'Turned sideways',
      'The same game across the width of the phone.',
      'A Social Scramble scorecard in landscape'),
    shot('guide/03_social_scramble/20_scorecard_time_up',430,1276,[430, 645],
      'Full time',
      'When the clock runs out the board locks and the score stops being tappable — the result is written through the dialog instead. This is the state a round ends in.',
      'A Social Scramble scorecard with its timer at zero and the score locked'),
    shot('guide/03_social_scramble/21_dialog_manual_score',430,932,[430, 860],
      'Setting the score by hand',
      'The same dialog the timer raises when it runs out while you are watching. It opens prefilled with whatever is on the board, so accepting the score as it stands is one tap — which is what the note is telling you.',
      'The manual score dialog with both sides prefilled'),
    shot('guide/03_social_scramble/22_dialog_override_result',430,932,[430, 860],
      'Correcting a finished game',
      'A score that is already recorded can still be rewritten, but never silently: the warning names what is about to be replaced. Mid-game the option is hidden altogether, so a live board cannot be overwritten by accident.',
      'The confirmation shown before replacing a recorded result'),
    shot('guide/03_social_scramble/17_splash_game_complete',430,1276,[430, 645],
      'The recap',
      'Completing a game hands you the result first. It waits for Continue rather than timing out, so nobody loses the moment to a stray tap.',
      'The game summary shown when a Social Scramble game completes'),
    shot('guide/03_social_scramble/18_dialog_game_complete',430,932,[430, 860],
      'What happens next',
      'Then the chooser: straight into the next game, or back to the schedule. If the board is still 0-0 it offers to set the score first, and Cancel leaves the finished game exactly as it is.',
      'The Game Complete dialog offering Start Next Game or Back to Schedule'),
    shot('guide/03_social_scramble/19_sheet_next_game',430,1276,[430, 645],
      'Picking the next game',
      'With one game left it goes straight there. With several it asks which, grouped by round, so a referee moving between courts can pick the one in front of them.',
      'The next-game picker listing upcoming games by round'),
    shot('guide/03_social_scramble/23_sheet_tournament_complete',430,978,[430, 645],
      'The end of the session',
      'When the last outstanding game is completed the tournament closes itself and the final standings arrive in place of the usual chooser.',
      'The tournament complete sheet with final standings'),
    shot('guide/03_social_scramble/24_scorecard_pace',430,1335,[430, 645],
      'Pace alerts on the scorecard',
      'Optional, switched on at setup. With them on the card carries the round against its planned slot, so a court that is running late says so while you can still do something about it.',
      'A Social Scramble scorecard showing its pace against schedule'),
    shot('guide/03_social_scramble/25_scorecard_pace_landscape',860,397,[860, 1600],
      'Pace, sideways',
...
'm-royal-rotation-hub': {
  ...
  blocks:[
    ...
    shot('guide/04_royal_rotation/03_setup_full',430,1318,[430, 645],
      'The whole setup page',
      'Every choice on one screen, scrolled out in full. Defaults are already sensible — in practice you set the people, the courts and the time, and leave the rest alone.',
      'The complete Royal Rotation setup page'),
    sect('Every setting on the page','i-south'),
...
'm-royal-rotation-run': {
  ...
  blocks:[
    ...
    shot('guide/04_royal_rotation/04_overview',430,932,[430, 860],
      'The tournament page',
      'Rounds and courts, with the queue behind each one. Teams are drawn fresh every round, so the teams pill is worth a look before play starts.',
      'A running Royal Rotation'),
    shot('guide/04_royal_rotation/06_overview_by_court',430,1463,[430, 645],
      'Read it by court',
      'The same session grouped by court.',
      'A Royal Rotation schedule grouped by court'),
    shot('guide/04_royal_rotation/15_court_allocation',430,932,[430, 645],
      'Court allocation',
      'Courts in play across the session, measured in rounds.',
      'The Royal Rotation court allocation grid'),
    shot('guide/04_royal_rotation/26_overview_pace_overdue',430,1142,[430, 645],
      'Running late, across the session',
      'With pace alerts on, every round is measured against its planned slot rather than only the one in front of you. That is where a session that has quietly slipped becomes visible — and early enough to shorten a round or open a court.',
...
'm-royal-rotation-score': {
  ...
  blocks:[
    ...
    shot('guide/04_royal_rotation/07_court_manual_assigning',430,1992,[430, 645],
      'Manual: choosing who comes on',
      'With manual assignment nobody is seated for you. The card offers the fairest pairings it can find on the left and the whole queue on the right — one tap on a suggestion starts that side immediately.',
      'A Royal Rotation court with the manual side picker open'),
    shot('guide/04_royal_rotation/08_court_manual_assigning_landscape',860,397,[860, 1600],
      'The picker, sideways',
      'Landscape keeps the suggestions and the roster side by side.',
      'The Royal Rotation manual picker in landscape'),
    shot('guide/04_royal_rotation/09_court_manual_running',430,1992,[430, 645],
      'Manual: the court running',
      'Once a side is seated the picker gives way to the score. There is no challenger queue here — in manual assignment you decide who comes on next, so the card shows only the game in front of you.',
      'A Royal Rotation court running under manual assignment'),
    shot('guide/04_royal_rotation/10_court_manual_running_landscape',860,397,[860, 1600],
      'Running, sideways',
      'The same court in landscape, with the eject rail on the edge.',
      'A running Royal Rotation court in landscape'),
    shot('guide/04_royal_rotation/11_court_automated',430,2131,[430, 645],
      'Automated: the queue picks',
      'Automated assignment fills the chain for you: who is on court, who challenges next, who is up after them. Re-roll if a pairing looks wrong, and the rest of the queue shuffles up behind it.',
      'A Royal Rotation court with the automated challenger queue'),
    shot('guide/04_royal_rotation/12_court_automated_landscape',860,397,[860, 1600],
      'The queue, sideways',
      'Landscape stacks the chain beside the score instead of above it.',
      'An automated Royal Rotation court in landscape'),
    shot('guide/04_royal_rotation/13_court_allplay',430,1780,[430, 645],
      'Auto All-Play',
      'All-Play adds a third tier and a scorekeeper drawn from outside the rotation, so everyone at the court has a job. It needs a fuller court than the other two modes — the queue is longer by a whole side.',
      'A Royal Rotation court in Auto All-Play with its scorekeeper'),
    shot('guide/04_royal_rotation/14_court_allplay_landscape',860,397,[860, 1600],
      'All-Play, sideways',
      'The full chain and the scorekeeper, laid out for the sideline.',
      'An Auto All-Play Royal Rotation court in landscape'),
    shot('guide/04_royal_rotation/23_court_time_up',430,932,[430, 645],
      'When the clock runs out',
      'A queue court has no "time is up" prompt to answer. The clock hitting zero closes the court outright and puts its ranking on screen — who won the court, and how everyone below them finished. Portrait only: landscape shows the same card, wider.',
      'A Royal Rotation court closed by its timer, showing the court ranking'),
    shot('guide/04_royal_rotation/24_scorecard_pace',430,2100,[430, 645],
      'Pace alerts on the court',
      'Optional, switched on at setup. The court then carries its round against the planned slot — a flag, the time it should have finished, and On track, Due or Overdue beside it.',
      'A Royal Rotation court showing its pace against schedule'),
    shot('guide/04_royal_rotation/25_scorecard_pace_landscape',860,397,[860, 1600],
      'Pace, sideways',
      'Landscape gives pace its own panel rather than folding it into the timer row, so the warning stays readable at arm\'s length.',
...
'm-royal-shuffle-hub': {
  ...
  blocks:[
    ...
    shot('guide/06_royal_shuffle/03_setup_full',430,1323,[430, 645],
      'The whole setup page',
      'Every choice on one screen, scrolled out in full. Defaults are already sensible — in practice you set the people, the courts and the time, and leave the rest alone.',
      'The complete Royal Shuffle setup page'),
    sect('Every setting on the page','i-south'),
...
'm-royal-shuffle-run': {
  ...
  blocks:[
    ...
    shot('guide/06_royal_shuffle/04_overview',430,932,[430, 860],
      'The tournament page',
      'Every court and the queue waiting on it. Partners are shuffled for each new turn on court, so nobody plays the whole session with the same person.',
      'A running Royal Shuffle'),
    shot('guide/06_royal_shuffle/06_overview_by_court',430,1351,[430, 645],
      'Read it by court',
      'The same session grouped by court.',
      'A Royal Shuffle schedule grouped by court'),
    shot('guide/06_royal_shuffle/15_court_allocation',430,932,[430, 645],
      'Court allocation',
      'Courts in play across the session. A queue mode honours the court count down to a minimum you set — below it the queue would stall rather than shorten.',
...
'm-royal-shuffle-score': {
  ...
  blocks:[
    ...
    shot('guide/06_royal_shuffle/07_court_manual_assigning',430,1093,[430, 645],
      'Manual: choosing who comes on',
      'With manual assignment nobody is seated for you. The card offers the fairest pairings it can find on the left and the whole queue on the right — one tap on a suggestion starts that side immediately.',
      'A Royal Shuffle court with the manual side picker open'),
    shot('guide/06_royal_shuffle/08_court_manual_assigning_landscape',860,397,[860, 1600],
      'The picker, sideways',
      'Landscape keeps the suggestions and the roster side by side.',
      'The Royal Shuffle manual picker in landscape'),
    shot('guide/06_royal_shuffle/09_court_manual_running',430,1093,[430, 645],
      'Manual: the court running',
      'Once a side is seated the picker gives way to the score. There is no challenger queue here — in manual assignment you decide who comes on next, so the card shows only the game in front of you.',
      'A Royal Shuffle court running under manual assignment'),
    shot('guide/06_royal_shuffle/10_court_manual_running_landscape',860,397,[860, 1600],
      'Running, sideways',
      'The same court in landscape, with the eject rail on the edge.',
      'A running Royal Shuffle court in landscape'),
    shot('guide/06_royal_shuffle/11_court_automated',430,1175,[430, 645],
      'Automated: the queue picks',
      'Automated assignment fills the chain for you: who is on court, who challenges next, who is up after them. Re-roll if a pairing looks wrong, and the rest of the queue shuffles up behind it.',
      'A Royal Shuffle court with the automated challenger queue'),
    shot('guide/06_royal_shuffle/12_court_automated_landscape',860,397,[860, 1600],
      'The queue, sideways',
      'Landscape stacks the chain beside the score instead of above it.',
      'An automated Royal Shuffle court in landscape'),
    shot('guide/06_royal_shuffle/13_court_allplay',430,1177,[430, 645],
      'Auto All-Play',
      'All-Play adds a third tier and a scorekeeper drawn from outside the rotation, so everyone at the court has a job. It needs a fuller court than the other two modes — the queue is longer by a whole side.',
      'A Royal Shuffle court in Auto All-Play with its scorekeeper'),
    shot('guide/06_royal_shuffle/14_court_allplay_landscape',860,397,[860, 1600],
      'All-Play, sideways',
      'The full chain and the scorekeeper, laid out for the sideline.',
      'An Auto All-Play Royal Shuffle court in landscape'),
    shot('guide/06_royal_shuffle/23_court_time_up',430,932,[430, 645],
      'When the clock runs out',
      'A queue court has no "time is up" prompt to answer. The clock hitting zero closes the court outright and puts its ranking on screen — who won the court, and how everyone below them finished. Portrait only: landscape shows the same card, wider.',
      'A Royal Shuffle court closed by its timer, showing the court ranking'),
    sect('What this card gives you','i-south'),
...
/* ── Doghouse Shuffles · the three steps ────────────────────────────────────── */

'm-doghouse-hub': {
  title:'Setting up a Doghouse Shuffle', route:'/guide/modes/doghouse/tournament-hub', icon:'i-trophy', parent:'m-doghouse',
  eyebrow:'Doghouse Shuffle · Tournament Hub',
  h1:['Setting up a ','Doghouse Shuffle'],
  lead:'Two numbers define a Doghouse Shuffle: how many side-outs it takes to get free, and how many losses put you out for good. Everything else is the usual — players, courts, time.',
  blocks:[
    panel('What you set',null,[
    ...
    shot('guide/07_doghouse_shuffle/03_setup_full',430,1323,[430, 645],
      'The whole setup page',
      'Every choice on one screen, scrolled out in full. Defaults are already sensible — in practice you set the people, the courts and the time, and leave the rest alone.',
      'The complete Doghouse Shuffle setup page'),
    sect('Every setting on the page','i-south'),
    opts('Doghouse Shuffle · Tournament Setup',[
      opt('Target Players','6, 8, 12, 16, 20, 24, 32 — or type any number','12 · 1 to 128',
        {what:'How many people you are planning for.',
         when:'Below the floor the mode needs: “Auto-Allplay works with fewer players, but rotation may feel clunky below {count} players.”',
         help:'“How many players you plan to have. Used to size the ‘fill random’ quick-add and to check your court/round settings make sense.”'}),
  ...
  /* ══ INBOX · migriert aus pages/modes/doghouse.html ══
     ...
     Inhalt in blocks steht. Sichtbar ueber SHOW_INBOX in render.js. ══ */
  inbox:[
    sect('From the Doghouse Shuffles page · to review','i-south'),
    fbox('i-trophy','The Doghouse Shuffles hub','New Tournament starts a session. Below it sits your history, each entry showing the date, the player count and how far it got.'),
  ],
},

'm-doghouse-run': {
  title:'Running a Doghouse Shuffle', route:'/guide/modes/doghouse/tournament', icon:'i-grid', parent:'m-doghouse',
  eyebrow:'Doghouse Shuffle · Running it',
  h1:['Running a ','Doghouse Shuffle'],
  ...
  blocks:[
    ...
    shot('guide/07_doghouse_shuffle/04_overview',430,932,[430, 860],
      'The tournament page',
      'Courts, queue and the two ways out of the doghouse — escape by reaching the target, or hit the loss limit and be ejected. The pills carry both thresholds.',
      'A running Doghouse Shuffle'),
    shot('guide/07_doghouse_shuffle/06_overview_by_court',430,1383,[430, 645],
      'Read it by court',
      'The same session grouped by court.',
      'A Doghouse Shuffle schedule grouped by court'),
    shot('guide/07_doghouse_shuffle/15_court_allocation',430,932,[430, 645],
      'Court allocation',
      'Courts in play across the session.',
      'The Doghouse Shuffle court allocation grid'),
    sect('Every pill on the page','i-south'),
    opts('Doghouse Shuffle · the pill row',[
      opt('“Standings”','The overall ranking — and the page where the ranking method itself can still be changed.','Opens the ranking page'),
      opt('“Allocation”','What each court is doing right now — beside the court count, which only says how many there are.','Opens the court screen'),
      opt('“{n} players”','The roster: add someone mid-session, send a player on a break, or swap one out.','Opens the sheet'),
...
'm-doghouse-score': {
  title:'Scoring a Doghouse Shuffle match', route:'/guide/modes/doghouse/scorecard', icon:'i-score', parent:'m-doghouse',
  eyebrow:'Doghouse Shuffle · Scorecard',
  h1:['Scoring a ','Doghouse Shuffle match'],
  ...
  blocks:[
    ...
    fbox('i-queue','Which card this is','The queue scorecard — the court, the challengers, and who is up next.'),
    shot('guide/07_doghouse_shuffle/07_court_manual_assigning',430,1093,[430, 645],
      'Manual: choosing who comes on',
      'With manual assignment nobody is seated for you. The card offers the fairest pairings it can find on the left and the whole queue on the right — one tap on a suggestion starts that side immediately.',
      'A Doghouse Shuffle court with the manual side picker open'),
    shot('guide/07_doghouse_shuffle/08_court_manual_assigning_landscape',860,397,[860, 1600],
      'The picker, sideways',
      'Landscape keeps the suggestions and the roster side by side.',
      'The Doghouse Shuffle manual picker in landscape'),
    shot('guide/07_doghouse_shuffle/09_court_manual_running',430,1093,[430, 645],
      'Manual: the court running',
      'Once a side is seated the picker gives way to the score. There is no challenger queue here — in manual assignment you decide who comes on next, so the card shows only the game in front of you.',
      'A Doghouse Shuffle court running under manual assignment'),
    shot('guide/07_doghouse_shuffle/10_court_manual_running_landscape',860,397,[860, 1600],
      'Running, sideways',
      'The same court in landscape, with the eject rail on the edge.',
      'A running Doghouse Shuffle court in landscape'),
    shot('guide/07_doghouse_shuffle/11_court_automated',430,1318,[430, 645],
      'Automated: the queue picks',
      'Automated assignment fills the chain for you: who is on court, who challenges next, who is up after them. Re-roll if a pairing looks wrong, and the rest of the queue shuffles up behind it.',
      'A Doghouse Shuffle court with the automated challenger queue'),
    shot('guide/07_doghouse_shuffle/12_court_automated_landscape',860,397,[860, 1600],
      'The queue, sideways',
      'Landscape stacks the chain beside the score instead of above it.',
      'An automated Doghouse Shuffle court in landscape'),
    shot('guide/07_doghouse_shuffle/13_court_allplay',430,1262,[430, 645],
      'Auto All-Play',
      'All-Play adds a third tier and a scorekeeper drawn from outside the rotation, so everyone at the court has a job. It needs a fuller court than the other two modes — the queue is longer by a whole side.',
      'A Doghouse Shuffle court in Auto All-Play with its scorekeeper'),
    shot('guide/07_doghouse_shuffle/14_court_allplay_landscape',860,397,[860, 1600],
      'All-Play, sideways',
      'The full chain and the scorekeeper, laid out for the sideline.',
      'An Auto All-Play Doghouse Shuffle court in landscape'),
    shot('guide/07_doghouse_shuffle/23_court_time_up',430,932,[430, 645],
      'When the clock runs out',
      'A queue court has no "time is up" prompt to answer. The clock hitting zero closes the court outright and puts its ranking on screen — who won the court, and how everyone below them finished. Portrait only: landscape shows the same card, wider.',
      'A Doghouse Shuffle court closed by its timer, showing the court ranking'),
    sect('What this card gives you','i-south'),
    opts('Doghouse Shuffle · the scorecard',[
...
'quick-game-hub': {
  ...
  lead:'Setting up a game takes about five seconds. Tell the sheet how many points you want to play to, and decide whether to pull in teams and players you already created or simply have TournaQ generate a random pairing. Either way you start right away — generated names can still be edited later on the scorecard itself, so naming the sides properly stays optional.',
  blocks:[
    panel('What you set',null,[
    ...
    shot('guide/02_quick_game/03_quick_start_sheet',430,932,[430, 860],
      'Starting a game',
      'Names, format, target score. Nothing is saved until the first point is scored, so this is a cheap thing to open.',
  ...
  inbox:[
    ...
    shot('guide/02_quick_game/04_quick_start_teams',430,932,[430, 860],
...
'quick-game-score': {
  ...
  lead:'Just start the game and tap the counters. The app tells you when to change sides, who is serving, and when the target score is reached — all from the settings you chose. See the sections below for more.',
  blocks:[
    panel('On the card',null,[
    ...
    fbox('i-check','Which card this is','The classic scorecard — sets, target points and side switches. The same one every team competition uses.'),
    shot('guide/02_quick_game/08_scorecard_three_sets',430,932,[430, 645],
      'A best-of-three, mid match',
      'One set each and the decider under way. The set strip keeps the finished sets in view while the live one is scored.',
      'A Quick Game scorecard in the deciding set'),
    shot('guide/02_quick_game/07_scorecard_landscape',860,397,[860, 1600],
      'Turned sideways',
  ...
  inbox:[
    ...
    shot('guide/02_quick_game/09_dialog_switch_sides',430,932,[430, 860],
      'Change ends',
    ...
    shot('guide/02_quick_game/10_dialog_game_won',430,932,[430, 860],
      'Game won',
    ...
    shot('guide/02_quick_game/02_games_full',430,1088,[430, 645],
      'Your history',
...
'm-doghouse': {
  title:'Doghouse Shuffles', route:'/guide/modes/doghouse', icon:'i-shield', parent:'queue-modes',
  eyebrow:'Queue mode · scramble competition',
       ...
       ['arena',1],
         ...
         ['brackets',2],
           ...
           ['queue-modes',3,'queue-modes@brackets'],
             ['m-royal-duo',4],['m-royal-duo-hub',5],['m-royal-duo-run',5],['m-royal-duo-score',5],
         ['scrambles',2],
